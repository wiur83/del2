...
module.exports = {
    ...
    getDataAll: async function() {
        return new Promise(resolve => {
            let arrayWords = [];
            db.all("SELECT * FROM data_all", async (err, row) => {
                if (err) {
                    console.log(err);
                    resolve(err);
                } else {
    ...
    setDataAll: async function(wordsListDataAll, userBackupId) {
        return new Promise(resolve => {
            for (let i = 0; i < wordsListDataAll.length; i++) {
                console.log(wordsListDataAll[i]["word"]);
                console.log(wordsListDataAll[i]["res_word"]);
                db.get("SELECT COUNT(*) AS total FROM data WHERE word = ? AND res_word = ? AND user_id = ?",
                wordsListDataAll[i]["word"],wordsListDataAll[i]["res_word"],userBackupId,(err, row) => {
                    if (err) {
                        resolve(err);
                    } else {
                        if (row.total == 0) {
                            //ord finns inte
                            db.run("INSERT INTO data (user_id, word, res_word, nr_of_tries) VALUES (?, ?, ?, ?)",
                                userBackupId, wordsListDataAll[i]["word"],wordsListDataAll[i]["res_word"], 1, (err) => {
                                if (err) {
                                    resolve(err);
                                }
                            });
    ...
    getAllData: async function() {
        return new Promise(resolve => {
            db.all("SELECT * FROM data", async (err, row) => {
                if (err) {
                    console.log(err);
                    resolve(err);
                } else {
    ...
    setAllData: async function(data) {
        return new Promise(resolve => {
            ...
            for (const property in data) {
              ...
              for (var i = 0; i < data[property].length; i++) {
                  ...
                  db.run("INSERT INTO data_all (word, res_word, nr_of_tries) VALUES (?, ?, ?)",
                      property, data[property][i],parseInt(1), (err) => {
                      if (err) {
                          resolve(err);
                      }
                  });
            ...
            // db.run("INSERT INTO data_all (user_id, score, total_score) VALUES (?, ?, ?)",
            //     userBackupId, parseInt(score),parseInt(counterWords), (err) => {
            //     if (err) {
            //         resolve(err);
            //     }
