...
// const url_adress = "http://me-api.thisisabad.site";
...
//register GET
router.get("/register", resetAuth, (req, res) => {
...
//login GET
router.get("/login", resetAuth, (req, res) => {
        ...
        await fetch(`${url_adress}/user/login-push`, {
            ...
            .then(async function (result) {
                if (result.msg == "error") {
                    ...
                    let words_data_all = await VoiceMethods.getDataAll()
                    console.log("------------------");
                    console.log(words_data_all);
                    console.log("------------------");

                    // await VoiceMethods.setUserWords(words, result.backup_id);
                    await VoiceMethods.setDataAll(words_data_all, result.backup_id);
...
router.get("/sort", async (req, res) => {
    let dataTable = await VoiceMethods.getAllData();
    console.log("--------");
    console.log(dataTable);
    console.log("--------");
    let NewObj = {};
    for (const property in dataTable) {
      console.log(property);
      console.log(dataTable[property]);
      let newArray = [];
      for (var i = 0; i < dataTable[property].length; i++) {
          if (newArray.includes(dataTable[property][i])) {
              console.log("duplicate");
          } else {
              newArray.push(dataTable[property][i]);
          }
      }

      NewObj[property] = newArray;

    }
    console.log("--newObj----");
    console.log(NewObj);
    console.log("--newObj----");
    await VoiceMethods.setAllData(NewObj);
